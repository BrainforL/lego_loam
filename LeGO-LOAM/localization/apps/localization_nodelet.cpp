...
public:
  ...
  void onInit() override {
    ...
    if(use_imu) {
      ...
      imu_sub = mt_nh.subscribe("/imu/data", 256, &LocalizationNodelet::imu_callback, this);
    }
    points_sub = mt_nh.subscribe("/points_raw", 5, &LocalizationNodelet::points_callback, this);
    globalmap_sub = nh.subscribe("/globalmap", 1, &LocalizationNodelet::globalmap_callback, this);
    initialpose_sub = nh.subscribe("/initialpose", 8, &LocalizationNodelet::initialpose_callback, this);
    
    localizer_pose_pub = nh.advertise<geometry_msgs::PoseStamped>("/localizer_pose", 5, false);
...
private:
  void initialize_params() {
    ...
    double ndt_resolution = private_nh.param<double>("ndt_resolution", 1.0);  //网格大小默认为1立方米
    boost::shared_ptr<pcl::VoxelGrid<PointT>> voxelgrid(new pcl::VoxelGrid<PointT>());
    voxelgrid->setLeafSize(downsample_resolution, downsample_resolution, downsample_resolution);
    downsample_filter = voxelgrid;

    pclomp::NormalDistributionsTransform<PointT, PointT>::Ptr ndt(new pclomp::NormalDistributionsTransform<PointT, PointT>());
    ndt->setTransformationEpsilon(0.01); //精度为1厘米
    ...
    if(private_nh.param<bool>("specify_init_pose", true)) {
      ...
      pose_estimator.reset(new localization::PoseEstimator(registration,
        ros::Time::now(),
...
private:
  ...
  void points_callback(const sensor_msgs::PointCloud2ConstPtr& points_msg) {
    ...
    publish_odometryAndlocalizerPose(points_msg->header.stamp, pose_estimator->matrix());
  }
  ...
  void initialpose_callback(const geometry_msgs::PoseWithCovarianceStampedConstPtr& pose_msg) {
    ...
    pose_estimator.reset(
          new localization::PoseEstimator(
            registration,
            ros::Time::now(),
   ...
   */
  void publish_odometryAndlocalizerPose(const ros::Time& stamp, const Eigen::Matrix4f& pose) {
    ...
    localizer_pose_msg.header.stamp = stamp;
    localizer_pose_msg.pose.position.x = pose(0, 3);
    localizer_pose_msg.pose.position.y = pose(1, 3);
    localizer_pose_msg.pose.position.z = pose(2, 3);
    localizer_pose_msg.pose.orientation = odom_trans.transform.rotation;
...
PLUGINLIB_EXPORT_CLASS(localization::LocalizationNodelet, nodelet::Nodelet)
